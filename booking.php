...
<?php 
    ...
    if(isset($_POST['book'])){
        ...
        if(empty($bplace)|| empty($bdate)|| empty($dur)|| empty($phno)|| empty($des)|| empty($rdate)){
            ...
            if($bdate<=$rdate){
                $dur_type = $_POST['dur_type']; // Get duration type
                if ($dur_type == "hour") {
                    $price = ($dur * ($carprice / 24)); // Assuming price per day, convert it to hourly rate
                } else {
                    $price = ($dur * $carprice); // Normal per day calculation
                }                
                $sql="INSERT INTO booking (CAR_ID, EMAIL, BOOK_PLACE, BOOK_DATE, DURATION_VALUE, DURATION_TYPE, PHONE_NUMBER, DESTINATION, PRICE, RETURN_DATE) 
                VALUES ($carid, '$uemail', '$bplace', '$bdate', $dur, '$dur_type', $phno, '$des', $price, '$rdate')";                        
    ...
    ?>
       ...
       <div class="hai">
        ...
        <form id="register" method="POST"  >
            ...
            <input type="number" name="dur" min="1" max="30" id="name" placeholder="Enter Rent Duration">
            <br><br>
